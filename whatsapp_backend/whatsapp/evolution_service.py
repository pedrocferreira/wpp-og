            ...
            if data.get('event') == 'messages.upsert':
                ...
                if message_type == 'conversation':
                    ...
                    def eh_agendamento(texto):
                        agendamento_palavras = ['agendar', 'marcar', 'consulta', 'horário', 'disponibilidade', 'quero agendar']
                        return any(palavra in texto.lower() for palavra in agendamento_palavras)

                    # Intercepta a intenção de agendamento ANTES do fluxo antigo
                    if eh_agendamento(content):
                        logger.info("Intenção de agendamento detectada pelo fluxo principal. Enviando link.")
                        
                        booking_link = f"http://155.133.22.207:4200/agendamento?whatsapp={phone}"
                        
                        resposta = f"""📅 *Agendamento de Consultas*
...
Após agendar, você receberá uma confirmação aqui mesmo! 😊"""
                        ...
                        logger.info('Mensagem de link de agendamento enviada e salva no banco')
                        return # Finaliza o processamento aqui, ignorando o fluxo antigo de state machine
